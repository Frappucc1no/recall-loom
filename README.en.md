...
|---|---|---|
| `v0.4.6.1` | Emergency v0.4.6-line bugfix: upgraded legacy receipt stores can append safely again; `record --plan` recognizes no-write confirmation language more accurately; blocked preflight no longer exposes an executable write command | Users already on v0.4.6 should update too; real projects no longer get stuck on historical receipt-contract identity drift, and recording plans are less misleading |
