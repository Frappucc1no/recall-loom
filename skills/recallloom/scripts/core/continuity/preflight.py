#!/usr/bin/env python3
"""Callable preflight/readiness evaluation over a consistent workspace snapshot.

Extracted from ``preflight_context_check.py::main`` (T050-03A, frozen landing map
§7.8 packet 03A): snapshot loading, freshness/revision reasoning,
write-readiness/tier computation, and digest computation. Pure decision code — no
CLI, no argparse, no subprocess, no persistent writes, no printing. The CLI helper
delegates here and projects the returned payload into its legacy JSON/human output
unchanged.
"""
...
from core.output.privacy import public_project_path, public_project_root_label
from core.output.user_status import preflight_user_summary
from core.protocol.contracts import DAILY_LOGS_DIRNAME, FILE_KEYS
from core.protocol.markers import parse_daily_log_scaffold_marker, parse_file_state_marker
...
from core.workspace.runtime import WorkspaceInfo, load_workspace_state, read_text


class PreflightSnapshotError(Exception):
    """Structured refusal for malformed managed files during preflight snapshot loading."""
...
class PreflightEvaluation:
    """Pure preflight/readiness outcome; adapters project it into output."""

    payload: dict
    append_date_review_required: bool
...
def evaluate_preflight(
    *,
    ...
    full: bool,
    startup_residue_report: dict | None = None,
) -> PreflightEvaluation:
    """Evaluate preflight/readiness over a consistent snapshot of ``workspace``.

    Pure: reads the workspace, computes the exact legacy preflight payload, and
    returns it with the flags the CLI adapter needs for human output. Raises
    ``PreflightSnapshotError`` for malformed managed files and lets
    ``DailyLogCursorError`` / ``ConfigContractError`` / ``OSError`` /
    ``UnicodeDecodeError`` propagate for the adapter's legacy failure projection.
    """
    ...
    if not summary_path.is_file():
        raise PreflightSnapshotError(f"Missing required file: {summary_path}")

    invalid_daily_logs = invalid_iso_like_daily_log_files(logs_dir)
    if invalid_daily_logs:
        raise PreflightSnapshotError(
    ...
    if summary_state is None:
        raise PreflightSnapshotError(
            f"Missing required file-state metadata marker: {summary_path}"
        )
    ...
    if context_brief_path.is_file():
        ...
        if context_brief_state is None:
            raise PreflightSnapshotError(
                f"Missing required file-state metadata marker: {context_brief_path}"
            )
    ...
    if update_protocol_path.is_file():
        ...
        if update_protocol_state is None:
            raise PreflightSnapshotError(
                f"Missing required file-state metadata marker: {update_protocol_path}"
            )
    ...
    workspace_artifact_scan_mode = "full" if full else "quick"
    ...
    latest_daily_log_text = latest_daily_log_full_text if full else ""
    ...
    latest_active_day = (
        date.fromisoformat(latest_daily_log.stem) if latest_daily_log is not None else None
    )
    ...
    project_time_policy_cues = (
        ...
        if full and update_protocol_path.is_file()
        else []
    )
    ...
    return PreflightEvaluation(
        payload=payload,
        append_date_review_required=append_date_review_required,
    )
